...
import { agentRecordToMirrorSnapshot, createMirrorEventHub, PI_SUBAGENTS_MIRROR_EVENT_TYPE } from "./mirror.js";
...
export default function (pi: ExtensionAPI) {
  ...
  const manager = new AgentManager((record) => {
    mirrorHub.publish({ type: PI_SUBAGENTS_MIRROR_EVENT_TYPE.AGENT_SNAPSHOT, agent: agentRecordToMirrorSnapshot(record) });
    mirrorHub.publish({ type: PI_SUBAGENTS_MIRROR_EVENT_TYPE.STATUS, agentId: record.id, status: record.status });
  ...
  const mirrorHub = createMirrorEventHub(() => manager.listAgents());
  const MIRROR_SERVICE_KEY = "__piSubagentsMirrorService";
  (globalThis as any)[MIRROR_SERVICE_KEY] = mirrorHub;
    ...
    execute: async (toolCallId, params, signal, onUpdate, ctx) => {
      ...
      if (runInBackground) {
        ...
        bgCallbacks.onSessionCreated = (session: any) => {
          ...
          if (rec) {
            mirrorHub.publish({ type: PI_SUBAGENTS_MIRROR_EVENT_TYPE.AGENT_SNAPSHOT, agent: agentRecordToMirrorSnapshot(rec) });
          }
        };
        const origBgOnTextDelta = bgCallbacks.onTextDelta;
        bgCallbacks.onTextDelta = (delta: string, fullText: string) => {
          origBgOnTextDelta(delta, fullText);
          mirrorHub.publish({ type: PI_SUBAGENTS_MIRROR_EVENT_TYPE.ASSISTANT_TEXT_DELTA, agentId: id, delta });
        };
      ...
      const origFgOnTextDelta = fgCallbacks.onTextDelta;
      fgCallbacks.onTextDelta = (delta: string, fullText: string) => {
        origFgOnTextDelta(delta, fullText);
        if (fgId) {
          mirrorHub.publish({ type: PI_SUBAGENTS_MIRROR_EVENT_TYPE.ASSISTANT_TEXT_DELTA, agentId: fgId, delta });
        }
      };
      ...
      fgCallbacks.onSessionCreated = (session: any) => {
        ...
        for (const a of manager.listAgents()) {
          if (a.session === session) {
            fgId = a.id;
            mirrorHub.publish({ type: PI_SUBAGENTS_MIRROR_EVENT_TYPE.AGENT_SNAPSHOT, agent: agentRecordToMirrorSnapshot(a) });
