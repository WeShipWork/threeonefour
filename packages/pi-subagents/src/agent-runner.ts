...
export async function runAgent(
  ...
  // Test harness support: Vitest/pnpm can load multiple @earendil-works/pi-ai
  // module instances, so a faux provider registered by the parent e2e harness
  // may not exist in the subagent session's api-registry. When the harness
  // publishes a bridge, register the inherited faux API in this extension
  // runtime too. Production runs never set this symbol.
  const fauxStreamBridge = (globalThis as Record<symbol, unknown>)[
    Symbol.for("pi-subagents:faux-stream-simple")
  ];
  if (model?.provider === "faux" && typeof fauxStreamBridge === "function") {
    const piAiUrl = import.meta.resolve("@earendil-works/pi-ai");
    const apiRegistryUrl = new URL("./api-registry.js", piAiUrl).href;
    const { registerApiProvider } = await import(apiRegistryUrl) as {
      registerApiProvider: (provider: unknown, sourceId: string) => void;
    };
    registerApiProvider({
      api: model.api,
      stream: fauxStreamBridge,
      streamSimple: fauxStreamBridge,
    }, `pi-subagents:faux:${model.api}`);
  }
