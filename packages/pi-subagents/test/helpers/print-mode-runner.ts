...
const FAUX_STREAM_BRIDGE_KEY = Symbol.for("pi-subagents:faux-stream-simple");
...
export async function runPrintMode(options: RunPrintModeOptions): Promise<PrintModeRun> {
  ...
  if (live) {
    ...
    model = faux.getModel();
    (globalThis as Record<symbol, unknown>)[FAUX_STREAM_BRIDGE_KEY] = streamSimple;
  ...
  const dispose = async () => {
    ...
    delete (globalThis as Record<symbol, unknown>)[FAUX_STREAM_BRIDGE_KEY];
